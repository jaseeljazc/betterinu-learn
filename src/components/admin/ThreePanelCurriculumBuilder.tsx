...
function getModuleTypeMeta(type: string) {
  if (type === "quiz") return { Icon: HelpCircle, color: "text-purple-500", bg: "bg-purple-100", label: "Quiz" };
  if (type === "assignment") return { Icon: ClipboardList, color: "text-orange-500", bg: "bg-orange-100", label: "Assignment" };
  return { Icon: FileText, color: "text-green-600", bg: "bg-green-100", label: "Lesson" };
}

// Dropdown to add Lesson / Quiz / Assignment to a Day
function AddModuleDropdown({ onAddLesson, onAddQuiz, onAddAssignment }: { onAddLesson: () => void; onAddQuiz: () => void; onAddAssignment: () => void }) {
    ...
    <div ref={ref} className="relative" onClick={(e) => e.stopPropagation()}>
      ...
      {open && (
        <div className="absolute right-0 top-full mt-1 z-50 bg-white border border-default rounded-lg shadow-lg min-w-[140px]">
          <button
            type="button"
            onClick={() => { onAddLesson(); setOpen(false); }}
          ...
          <button
            type="button"
            onClick={() => { onAddQuiz(); setOpen(false); }}
            className="flex items-center gap-2 w-full px-3 py-2 text-xs hover:bg-subtle transition-colors text-left"
          >
          ...
          <button
            type="button"
            onClick={() => { onAddAssignment(); setOpen(false); }}
            className="flex items-center gap-2 w-full px-3 py-2 text-xs hover:bg-subtle transition-colors text-left"
          >
  ...
  return (
    <div
      ...
      className={`group/lesson flex items-center gap-1.5 px-1.5 py-1 rounded-md cursor-pointer transition-colors ${
        isActive ? "bg-primary/10 text-primary font-medium" : "hover:bg-subtle text-muted-foreground hover:text-foreground"
      }`}
    >
      ...
      {editing ? (
        <input
          ...
          onKeyDown={(e) => {
            ...
            if (e.key === "Escape") { setDraft(mod.title || ""); setEditing(false); }
          }}
          onBlur={(e) => {
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <span className="flex-1 truncate text-xs">{mod.title || "Untitled Lesson"}</span>
          </TooltipTrigger>
          <TooltipContent side="right">
            {mod.title || "Untitled Lesson"}
          </TooltipContent>
        </Tooltip>
      )}
      
      <div className={`flex items-center gap-0.5 ${editing ? "flex" : "opacity-0 group-hover/lesson:opacity-100"} transition-opacity`}>
        {editing ? (
          <>
            <button type="button" onClick={(e) => { e.stopPropagation(); commitRename(); }} className="p-0.5 text-primary hover:text-primary"><Check className="size-3" /></button>
            <button type="button" onClick={(e) => { e.stopPropagation(); setDraft(mod.title || ""); setEditing(false); }} className="p-0.5 text-muted hover:text-foreground"><X className="size-3" /></button>
          </>
        ) : (
          <>
            <button type="button" onClick={(e) => { e.stopPropagation(); setDraft(mod.title || ""); setEditing(true); }} className="p-0.5 text-muted hover:text-primary"><Pencil className="size-3" /></button>
            <button type="button" onClick={(e) => { e.stopPropagation(); onRemove(); }} className="p-0.5 text-muted hover:text-red-500"><Trash2 className="size-3" /></button>
          </>
        )}
      </div>
    </div>
...
function SortableDayItem({ day, wIdx, dIdx, activeNode, onRenameDay, onRemoveDay, onAddLesson, onAddQuiz, onAddAssignment, onSelectLesson, onRenameLesson, onRemoveLesson }: any) {
  ...
  return (
    <div ref={setNodeRef} style={style} className="bg-surface rounded-md border border-transparent hover:border-default transition-colors pb-1">
      <div
        className={`group/day flex items-center gap-1.5 px-1 py-1 rounded-md transition-colors ${
          activeNode?.wIdx === wIdx && activeNode?.dIdx === dIdx && activeNode?.mIdx === undefined ? "bg-primary/5 text-primary font-medium" : "hover:bg-subtle text-foreground"
        }`}
      >
        <button type="button" {...attributes} {...listeners} className="shrink-0 text-muted hover:text-primary cursor-grab active:cursor-grabbing p-0.5" onClick={(e) => e.stopPropagation()}>
          <GripVertical className="size-3.5" />
        </button>
        <button type="button" className="text-muted hover:text-foreground shrink-0" onClick={(e) => { e.stopPropagation(); setExpanded(!expanded); }}>
        ...
        {editing ? (
          <input
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter") commitRename();
              if (e.key === "Escape") { setDraft(day.title || day.label || ""); setEditing(false); }
            }}
            onBlur={(e) => {
              if (!e.relatedTarget || !(e.currentTarget.parentElement?.contains(e.relatedTarget as Node))) {
                commitRename();
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <span className="flex-1 truncate text-sm font-semibold">{day.title || day.label || "Untitled Day"}</span>
            </TooltipTrigger>
            <TooltipContent side="right">
              {day.title || day.label || "Untitled Day"}
            </TooltipContent>
          </Tooltip>
        )}

        <div className={`flex items-center gap-0.5 ${editing ? "flex" : "opacity-0 group-hover/day:opacity-100"} transition-opacity`}>
          {editing ? (
            <>
              <button type="button" onClick={(e) => { e.stopPropagation(); commitRename(); }} className="p-0.5 text-primary hover:text-primary"><Check className="size-3.5" /></button>
              <button type="button" onClick={(e) => { e.stopPropagation(); setDraft(day.title || day.label || ""); setEditing(false); }} className="p-0.5 text-muted hover:text-foreground"><X className="size-3.5" /></button>
            </>
          ) : (
            <>
              <AddModuleDropdown
            onAddLesson={() => { onAddLesson(); setExpanded(true); }}
            onAddQuiz={() => { onAddQuiz(); setExpanded(true); }}
            onAddAssignment={() => { onAddAssignment(); setExpanded(true); }}
          />
              <button type="button" onClick={(e) => { e.stopPropagation(); setDraft(day.title || day.label || ""); setEditing(true); }} className="p-0.5 text-muted hover:text-primary"><Pencil className="size-3.5" /></button>
              <button type="button" onClick={(e) => { e.stopPropagation(); onRemoveDay(); }} className="p-0.5 text-muted hover:text-red-500"><Trash2 className="size-3.5" /></button>
            </>
          )}
        </div>
      </div>
        ...
        <div className="pl-6 pr-1 py-1 space-y-0.5 border-l border-default ml-2.5">
          {day.subModules?.map((mod: any, mIdx: number) => (
             <LessonItem 
               key={mod.id} 
               mod={mod} 
               isActive={activeNode?.wIdx === wIdx && activeNode?.dIdx === dIdx && activeNode?.mIdx === mIdx} 
          ...
          {(!day.subModules || day.subModules.length === 0) && (
            <p className="text-[10px] text-muted italic ml-1.5 mt-1">No lessons</p>
          )}
        </div>
      )}
    </div>
...
function WeekSection({ week, wIdx, activeNode, onAddDay, onRenameWeek, onRemoveWeek, onSelectDay, onRenameDay, onRemoveDay, onDragEndDays, onAddLesson, onAddQuiz, onAddAssignment, onSelectLesson, onRenameLesson, onRemoveLesson }: any) {
  const [editingWeek, setEditingWeek] = React.useState(false);
  const [weekDraft, setWeekDraft] = React.useState(week.title || `Week ${wIdx + 1}`);
  ...
  return (
    <div className="space-y-0.5">
          ...
          <>
            <input
              ...
              onKeyDown={(e) => {
                ...
                if (e.key === "Escape") { setWeekDraft(week.title || `Week ${wIdx + 1}`); setEditingWeek(false); }
              }}
              onBlur={(e) => {
                if (!e.relatedTarget || !(e.currentTarget.parentElement?.contains(e.relatedTarget as Node))) {
                  commitWeekRename();
            ...
            />
            <button type="button" onClick={commitWeekRename} className="p-0.5 text-primary hover:text-primary shrink-0">
              <Check className="size-3.5" />
            </button>
            <button type="button" onClick={() => { setWeekDraft(week.title || `Week ${wIdx + 1}`); setEditingWeek(false); }} className="p-0.5 text-muted shrink-0">
              <X className="size-3.5" />
            </button>
          </>
        ) : (
          <>
            <span className="flex-1 text-xs font-bold uppercase text-muted tracking-widest truncate">
              {week.title || `Week ${wIdx + 1}`}
            </span>
            <div className="flex items-center gap-0.5 opacity-0 group-hover/week:opacity-100 transition-opacity">
              <button type="button" onClick={() => { setWeekDraft(week.title || `Week ${wIdx + 1}`); setEditingWeek(true); }}
                className="p-0.5 text-muted hover:text-primary" title="Rename week">
                <Pencil className="size-3.5" />
              </button>
              <button type="button" onClick={onAddDay}
                className="p-0.5 text-muted hover:text-primary" title="Add day">
                <Plus className="size-3.5" />
              </button>
              <button type="button" onClick={onRemoveWeek}
                className="p-0.5 text-muted hover:text-red-500" title="Delete week">
        ...
        <SortableContext items={week.days.map((d: any) => d.id)} strategy={verticalListSortingStrategy}>
          {week.days.map((day: any, dIdx: number) => (
            <SortableDayItem
              ...
              onRenameLesson={(mIdx: number, title: string) => onRenameLesson(dIdx, mIdx, title)}
      ...
      {week.days.length === 0 && (
        <p className="text-xs text-muted text-center py-2 italic">No days yet</p>
      )}
    </div>
  );
}
...
function SortableSectionItem({ section, onClick, isActive, onRemove }: any) {
  ...
  return (
    ...
    >
      ...
      <div 
        className="absolute -left-10 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center p-2 text-muted hover:text-foreground"
      >
        <button type="button" {...attributes} {...listeners} className="cursor-grab active:cursor-grabbing p-1 bg-surface border border-default rounded-md shadow-sm">
      ...
      <div className="absolute -right-10 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center p-2">
        <button type="button" onClick={(e) => { e.stopPropagation(); onRemove(); }} className="text-muted hover:text-red-500 p-1 bg-surface border border-default rounded-md shadow-sm">
...
// --- MAIN COMPONENT ---
export function ThreePanelCurriculumBuilder({ form, update, onSave, onCancel, saving }: any) {
  ...
  const activeModule = activeNode && activeNode.mIdx !== undefined ? activeDay?.subModules?.[activeNode.mIdx] : activeDay?.subModules?.[0]; 
  const activeSections = activeModule?.sections || [];

  // Safely get active section
  const activeSection = activeSections.find((s: any) => s.id === activeSectionId);
  const activeSectionIndex = activeSections.findIndex((s: any) => s.id === activeSectionId);
  ...
  const removeLesson = (wIdx: number, dIdx: number, mIdx: number) => {
    ...
    update("curriculum", next);
    if (activeNode?.wIdx === wIdx && activeNode?.dIdx === dIdx && activeNode?.mIdx === mIdx) {
      setActiveNode(null);
      setActiveSectionId(null);
    } else if (activeNode?.wIdx === wIdx && activeNode?.dIdx === dIdx && activeNode.mIdx && activeNode.mIdx > mIdx) {
  ...
  const handleDragEndDays = (e: DragEndEvent, wIdx: number) => {
    ...
    if (over && active.id !== over.id) {
      ...
      if (activeNode && activeNode.wIdx === wIdx && activeNode.dIdx === oldIdx) {
  ...
  return (
    <TooltipProvider>
      ...
      <div className={`flex flex-col border-r border-default transition-all duration-300 ${sidebarCollapsed ? "w-16" : "w-72"} bg-surface shrink-0 relative z-20`}>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto p-2 space-y-3">
          {!sidebarCollapsed && form.curriculum?.map((week: any, wIdx: number) => {
            return (
              <WeekSection
                key={week.id}
                week={week}
          ...
          {!sidebarCollapsed && (!form.curriculum || form.curriculum.length === 0) && (
        ...
        <div className="p-4 border-b border-default bg-surface flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-display font-bold">
              {activeDay && activeModule ? `${activeDay.title || activeDay.label} / ${activeModule.title}` : activeDay ? `${activeDay.title || activeDay.label} - Content` : "Select a Lesson"}
            </h2>
          </div>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto p-6 relative">
          {!activeDay ? (
            <div className="h-full flex flex-col items-center justify-center text-muted">
              <p>Select a lesson from the left sidebar to edit its content.</p>
            </div>
            ...
            <div className="max-w-3xl mx-auto pb-20 space-y-5">
              ...
              <div className="rounded-2xl border border-default bg-surface p-6 shadow-sm">
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  ...
                  {(activeModule.assignmentData?.allowedSubmissionTypes || []).map((t: string) => (
                    <span key={t} className="rounded-full bg-surface border border-default px-2.5 py-1 text-[10px] font-bold text-muted uppercase tracking-wider">
                      {t}
                    </span>
                  ))}
                </div>
                ...
                {activeModule.assignmentData?.instructions ? (
                  <div
                    ...
                    dangerouslySetInnerHTML={{ __html: activeModule.assignmentData.instructions }}
                  />
                ) : (
              ...
              <div className="rounded-2xl border border-default bg-white p-6 shadow-sm">
                ...
                <div className="space-y-3 opacity-60 pointer-events-none">
                  ...
                  {((activeModule.assignmentData?.allowedSubmissionTypes || []).includes("file") ||
                    (activeModule.assignmentData?.allowedSubmissionTypes || []).includes("image")) && (
                          ...
                          <div className="bg-surface border border-default shadow-sm rounded-full p-1 flex items-center gap-1 group/add">
                            ...
                            <div className="hidden group-hover/add:flex items-center gap-1">
                              {SECTION_TYPES.map((t) => (
                                <button type="button"
                                  key={t.type}
                                  onClick={(e) => { e.stopPropagation(); addSection(t.type, idx); }}
          ...
          {activeModule?.type === "assignment" ? (
            <AssignmentModuleEditor
              ...
              onChange={(data) => {
                ...
                if (data.title) next[activeNode.wIdx].days[activeNode.dIdx].subModules[activeNode.mIdx ?? 0].title = data.title;
                update("curriculum", next);
              }}
            />
          ) : !activeSection ? (
                 ...
                 <div className="space-y-3">
                   ...
                   <div>
                     ...
                     <FileUploader 
                       ...
                       files={activeSection.url ? [{ url: activeSection.url, name: "image", type: "image/*" }] : []}
                       onChange={(files) => { if(files.length) updateSection({ url: files[0].url }) }}
                       role="admin"
                     />
                   </div>
                   <div>
                   ...
                   <div>
                     ...
                     <div className="flex bg-surface rounded-lg p-1 border border-default">
                       <button type="button" 
                         onClick={() => updateSection({ captionAlign: "left" })}
                       ...
                       >Left</button>
                       <button type="button" 
                         onClick={() => updateSection({ captionAlign: "center" })}
                       ...
                       >Center</button>
                       <button type="button" 
                         onClick={() => updateSection({ captionAlign: "right" })}
                 ...
                 <div className="space-y-3">
                   <div>
                     ...
                     <input 
                       ...
                       onChange={(e) => updateSection({ filename: e.target.value })} 
                     />
                   </div>
                   <div>
                     ...
                     <FileUploader 
                       ...
                       files={activeSection.url ? [{ url: activeSection.url, name: activeSection.filename || "document.pdf", type: "application/pdf" }] : []}
                       onChange={(files) => { 
                         if(files.length) {
                            updateSection({ url: files[0].url, filename: files[0].name });
                         }
                       }}
                       role="admin"
                     />
                 ...
                 <div className="space-y-3">
                   ...
                   <div>
                     ...
                     <select 
                       ...
                       onChange={(e) => updateSection({ submissionType: e.target.value })} 
                     >
                   ...
                   <div>
                     ...
                     <input 
                       ...
                       onChange={(e) => updateSection({ deadline: e.target.value })} 
                     />
                   </div>
                 </div>
               )}
