...
function defaultColCell(type: string): any {
  switch (type) {
    ...
    default:
      return { id: uid(), type: "rich_text", content: "" };
  }
}
...
function SectionEditor({
  ...
  const inputCls =
    "w-full rounded-lg border border-default bg-surface px-3 py-2 text-sm outline-none focus:border-primary focus:ring-1 focus:ring-primary/10 transition-colors";
  const labelCls =
    "block text-[10px] font-bold uppercase tracking-widest text-muted mb-1";
          ...
          <div>
            ...
            <div className="space-y-2">
              ...
              <FileUploader
                ...
                files={
                  section.url
                    ? [{ url: section.url, name: "image", type: "image/*" }]
                    : []
                }
                onChange={(files) => {
        ...
        <div className="space-y-3">
          <div>
            ...
            <p className="mt-1 text-[10px] text-muted">
              YouTube: https://youtube.com/watch?v=ID &nbsp;|&nbsp; Vimeo:
              https://vimeo.com/ID
            </p>
          </div>
          <div>
          ...
          <div>
            ...
            <div className="space-y-2">
              ...
              <FileUploader
                ...
                files={
                  section.url
                    ? [
                        {
                          url: section.url,
                          name: section.filename ?? "document.pdf",
                ...
                onChange={(files) => {
                  if (files.length)
                    patch({ url: files[0].url, filename: files[0].name });
                }}
                role="admin"
              />
            </div>
    ...
    default:
      return (
        <div className="p-3 bg-red-50 text-red-600 rounded-lg text-sm border border-red-200">
          <strong>Unsupported Section Type:</strong> "{(section as any).type}" —
          delete and recreate.
        </div>
      );
  }
...
function ColumnsEditor({
  ...
  const inputCls =
    "w-full rounded-lg border border-default bg-surface px-3 py-2 text-sm outline-none focus:border-primary focus:ring-1 focus:ring-primary/10";
  const labelCls =
    "block text-[10px] font-bold uppercase tracking-widest text-muted mb-1";
  ...
  function changeColType(i: number, type: string) {
    const next = cols.map((c, idx) =>
      idx === i ? { ...defaultColCell(type), id: c.id } : c,
    );
    onChange({ cols: next });
  }
    ...
    <div className="space-y-4">
      ...
      >
        {Array.from({ length: count }).map((_, i) => {
          ...
          const meta =
            COL_TYPES.find((t) => t.type === col.type) || COL_TYPES[0];
          return (
            <div
              key={col.id || i}
              className="rounded-xl border border-default bg-white overflow-hidden"
            >
              {/* Col header */}
              <div className="flex items-center gap-2 bg-surface px-3 py-2 border-b border-default">
                ...
                <Select
                  value={col.type}
                  onValueChange={(v) => changeColType(i, v)}
                >
                  ...
                  <div className="space-y-2">
                    {col.url && (
                      <Image
                        ...
                        height={144}
                        unoptimized
                        className="max-h-36 w-full object-contain rounded border border-default"
                      />
                    )}
                    <input
                      value={col.url || ""}
                      onChange={(e) => updateCol(i, { url: e.target.value })}
                    ...
                    <input
                      value={col.caption || ""}
                      onChange={(e) =>
                        updateCol(i, { caption: e.target.value })
                      }
                    ...
                    <Select
                      value={col.size || "full"}
                      onValueChange={(v) => updateCol(i, { size: v })}
                    >
                      <SelectTrigger className={inputCls}>
                        <SelectValue />
                      </SelectTrigger>
                ...
                {col.type === "pdf" && (
                  <div className="space-y-2">
                    ...
                    <input
                      value={col.filename || ""}
                      onChange={(e) =>
                        updateCol(i, { filename: e.target.value })
                      }
                ...
                {col.type === "link" && (
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        updateCol(i, { description: e.target.value })
                      }
                    ...
                    <div className="space-y-1">
                      ...
                      <FileUploader
                        ...
                        onChange={(files) => {
                          if (files.length)
                            updateCol(i, { thumbnailUrl: files[0].url });
                        }}
                        role="admin"
                      />
                    </div>
      ...
      return (
        <div
          ...
          dangerouslySetInnerHTML={{
            __html:
              section.content ||
              "<p className='text-muted italic'>Empty rich text</p>",
          }}
        />
      );
    case "image":
        ...
        <div className="flex flex-col items-center gap-2">
          {section.url ? (
            <Image
              ...
              height={128}
              unoptimized
              className="max-h-32 object-contain rounded-md border border-default"
            />
          ) : (
            <div className="h-24 w-40 bg-subtle border border-dashed rounded flex items-center justify-center text-muted text-xs">
              No image URL
            </div>
          )}
          {section.caption && (
      ...
      return (
        <div className="flex flex-col gap-1 bg-teal-50 p-3 rounded-lg border border-teal-100">
          <span className="font-semibold text-sm text-teal-900">
            {section.title || "Untitled Link"}
          </span>
          {section.description && (
            <p className="text-xs text-teal-800">{section.description}</p>
          )}
          <a
            href={section.url}
            target="_blank"
            rel="noreferrer"
            className="text-[10px] text-teal-600 hover:underline break-all"
          >
            {section.url || "No URL"}
          </a>
        </div>
      ...
      return (
        ...
        >
          {cols.map((col: any, i: number) => (
            <div
              key={col.id || i}
              className="rounded-lg border border-dashed border-default bg-subtle p-2 text-[10px] text-muted text-center"
            >
              ...
              {col.content ? (
                <div
                  className="text-left text-xs line-clamp-2 text-foreground"
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 bg-surface px-3 py-2 border-b border-default rounded-t-xl">
        ...
        {/* Type badge */}
        <span
          className={`flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider ${meta.color}`}
        >
      ...
      {sections.length === 0 && (
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-default py-8 text-center text-muted">
          ...
          <p className="text-xs mt-1">
            Add a section below to build your lesson.
          </p>
        </div>
      )}
