...
function StandaloneAssignmentsContent() {
  ...
  const initialTab =
    searchParams.get("tab") === "submissions" ? "submissions" : "assignments";
  ...
  const [newAssignmentData, setNewAssignmentData] =
    useState<AssignmentSubModuleData>({
      title: "",
      instructions: "",
  ...
  async function handleDelete(id: string) {
    if (
      !confirm(
        "Delete this assignment? All student submissions will also be removed.",
      )
    )
      return;
    setDeleting(id);
    await fetch(`/api/admin/standalone-assignments/${id}`, {
      method: "DELETE",
      credentials: "include",
    });
    setDeleting(null);
      ...
      <div className="mb-8 flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-secondary">
            Create and manage assignments that are independent of course
            curriculum.
          </p>
        </div>
        <button
            ...
            {(["all", "course", "common"] as const).map((f) => (
              ...
              >
                {f === "all"
                  ...
                  : f === "common"
                    ? `Common (${assignments.filter((a) => a.scope === "common").length})`
                    : `Course-Specific (${assignments.filter((a) => a.scope === "course").length})`}
              </button>
            ))}
          </div>
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-24 text-center rounded-2xl border border-dashed border-default bg-white">
              ...
              <p className="text-xs text-muted mt-1">
                Click "New Assignment" to create your first standalone task.
              </p>
            </div>
          ) : (
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
              {filtered.map((a) => (
                <div
                  key={a.id}
                  className="group relative rounded-2xl border border-default bg-white p-5 shadow-sm hover:shadow-md transition-all hover:border-primary/20"
                >
                  {/* Scope badge */}
                  ...
                  <div className="flex items-center gap-3 text-[10px] text-muted mb-4">
                    <span className="flex items-center gap-1">
                      <Users className="size-3" /> {a.student_count} assigned
                    </span>
                    {a.due_date && (
                      <span className="flex items-center gap-1">
                        <Clock className="size-3" /> Due {fmtDate(a.due_date)}
                      </span>
                    )}
                  </div>
          ...
          <div className="w-full max-w-4xl bg-white rounded-2xl shadow-2xl my-8">
            <div className="flex items-center justify-between px-6 py-4 border-b border-default">
              ...
              <button
                ...
                className="p-2 text-muted hover:bg-subtle rounded-full transition-colors"
              >
                <X className="size-5" />
              </button>
            </div>
            ...
            <div className="p-6">
              <div className="grid sm:grid-cols-2 gap-6 mb-8">
                ...
                <div>
                  ...
                  <div className="flex bg-subtle p-1 rounded-lg">
                    <button
                      onClick={() => setNewScope("course")}
                      className={`flex-1 rounded-md py-1.5 text-xs font-bold transition-colors ${newScope === "course" ? "bg-white shadow-sm text-primary" : "text-secondary hover:text-foreground"}`}
                    >
                      Course-Specific
                    </button>
                    <button
                      onClick={() => setNewScope("common")}
                      className={`flex-1 rounded-md py-1.5 text-xs font-bold transition-colors ${newScope === "common" ? "bg-white shadow-sm text-primary" : "text-secondary hover:text-foreground"}`}
                    >
                      Common Task
                    </button>
                  </div>
              ...
              <div className="flex justify-end gap-3 pt-6 border-t border-default">
                <button
                  ...
                  className="px-5 py-2.5 text-sm font-bold text-secondary hover:bg-subtle rounded-xl transition-colors"
                >
                  Cancel
                </button>
                <button
                  ...
                  disabled={creating}
                  className="flex items-center gap-2 bg-primary text-white px-6 py-2.5 rounded-xl text-sm font-bold hover:opacity-90 disabled:opacity-50 transition-opacity"
                >
                  {creating ? (
