...
import { CheckCircle2, Clock, Eye, X, XCircle, ClipboardList, Globe, BookOpen } from "lucide-react";
...
interface Submission {
  ...
  assignment_title: string; assignment_instructions: string;
  assignment_files: any[]; course_title: string | null;
  scope: string; submitted_text: string;
  submitted_files: { url: string; name: string; type: string }[];
  submitted_at: string; status: "pending" | "approved" | "rejected";
  feedback?: string; reviewed_at?: string;
}
    ...
    <div className="w-full">
      <div className="flex gap-2 mb-6 flex-wrap">
        {(["all", "pending", "approved", "rejected"] as const).map((f) => (
          <button key={f} onClick={() => setFilter(f)}
            className={`rounded-full px-4 py-1.5 text-xs font-bold capitalize transition-colors border ${filter === f ? "bg-primary text-white border-primary" : "bg-white text-secondary border-default hover:border-primary hover:text-primary"}`}>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-24 text-center rounded-2xl border border-dashed border-default bg-white">
          ...
          <p className="text-xs text-muted mt-1">Standalone task submissions will appear here.</p>
        </div>
      ) : (
              ...
              {filtered.map((sub) => {
                ...
                return (
                  <tr key={sub.id} className="hover:bg-subtle/50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <span className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[9px] font-bold uppercase ${sub.scope === "common" ? "bg-purple-50 border-purple-200 text-purple-700" : "bg-blue-50 border-blue-200 text-blue-700"}`}>
                        {sub.scope === "common" ? <><Globe className="size-3" />Common</> : <><BookOpen className="size-3" />Course</>}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-secondary text-xs">{new Date(sub.submitted_at).toLocaleString()}</td>
                    <td className="px-4 py-3">
          ...
          <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl bg-white shadow-2xl border border-default">
            <div className="flex items-center justify-between px-6 py-4 border-b border-default">
              <div>
                ...
                <h2 className="font-display text-lg font-bold text-foreground">{selected.assignment_title}</h2>
                <p className="text-xs text-muted">{selected.student_name} · {selected.student_email}</p>
              </div>
              <button onClick={() => setSelected(null)} className="rounded-full p-2 text-muted hover:bg-subtle hover:text-foreground transition-colors">
                <X className="size-5" />
              </button>
            </div>
            <div className="p-6 space-y-5">
              {selected.assignment_instructions && (
                <div>
                  <p className="text-xs font-bold uppercase tracking-widest text-muted mb-2">Assignment Instructions</p>
                  <div className="rounded-xl border border-default bg-subtle p-4 text-sm text-foreground rich-content leading-relaxed" dangerouslySetInnerHTML={{ __html: selected.assignment_instructions }} />
                </div>
              )}
              {(selected.assignment_files || []).length > 0 && <FileViewer files={selected.assignment_files} title="Reference Materials" />}
              <div>
              ...
              {actionError && <p className="text-xs text-red-600 font-semibold bg-red-50 border border-red-200 rounded-lg px-3 py-2">{actionError}</p>}
              <div className="flex items-center gap-3 pt-2 border-t border-default">
                <button onClick={() => handleAction("approve")} disabled={acting || selected.status === "approved"}
                  className="flex-1 flex items-center justify-center gap-2 rounded-xl bg-primary py-2.5 text-sm font-bold text-white hover:opacity-90 disabled:opacity-50">
                  {acting ? <RoboLoader size="xs" /> : <CheckCircle2 className="size-4" />} Approve
                </button>
                <button onClick={() => handleAction("reject")} disabled={acting || selected.status === "approved"}
                  className="flex-1 flex items-center justify-center gap-2 rounded-xl border border-red-300 bg-red-50 py-2.5 text-sm font-bold text-red-600 hover:bg-red-100 disabled:opacity-50">
                  {acting ? <RoboLoader size="xs" /> : <XCircle className="size-4" />} Reject
                </button>
              </div>
            </div>
          </div>
