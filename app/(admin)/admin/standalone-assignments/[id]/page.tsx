    ...
    <div className="rounded-xl border border-default bg-white overflow-hidden">
      ...
      <div className="flex items-center gap-3 p-3">
        ...
        <div className="flex items-center gap-2 shrink-0">
          {cfg ? (
            <span
              className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[9px] font-bold uppercase ${cfg.cls}`}
            >
          ...
          <div className="flex items-center justify-between flex-wrap gap-2">
            ...
            {s.reviewed_at && (
              <p className="text-[9px] text-muted">
                Reviewed {fmtDate(s.reviewed_at)}
              </p>
            )}
          </div>
...
export default function StandaloneAssignmentDetailPage() {
  ...
  const [editData, setEditData] = useState<AssignmentSubModuleData | null>(
    null,
  );
  const [editTitle, setEditTitle] = useState("");
  const [studentFilter, setStudentFilter] = useState<
    "all" | "submitted" | "pending" | "approved" | "rejected" | "todo"
  >("all");
  ...
  async function load() {
    ...
    try {
      const [aRes, sRes, allRes] = await Promise.all([
        fetch(`/api/admin/standalone-assignments/${id}`, {
          credentials: "include",
        }),
        fetch(`/api/admin/standalone-assignments/${id}/students`, {
          credentials: "include",
        }),
        fetch("/api/admin/students", { credentials: "include" }),
      ]);
      const [aData, sData, allData] = await Promise.all([
        aRes.json(),
        sRes.json(),
        allRes.json(),
      ]);
    ...
    try {
      const res = await fetch(`/api/admin/standalone-assignments/${id}`, {
        ...
        body: JSON.stringify({
          ...
          totalMarks: editData.totalMarks || null,
          allowedSubmissionTypes: editData.allowedSubmissionTypes,
          attachedFiles: editData.attachedFiles || [],
  ...
  if (!assignment)
    return (
      <div className="flex flex-col items-center justify-center h-64 gap-3">
        ...
        <Link
          href="/admin/standalone-assignments"
  ...
  const inputCls =
    "w-full rounded-lg border border-default bg-white px-3 py-2 text-sm outline-none focus:border-primary focus:ring-1 focus:ring-primary/10";
      ...
      <div className="mb-6">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          ...
          {!editing && (
            <button
              onClick={startEdit}
              className="flex items-center gap-2 rounded-xl border border-default bg-white px-4 py-2 text-sm font-semibold text-secondary hover:text-primary transition-colors shadow-sm"
            >
            ...
            <div className="rounded-2xl border border-default bg-white p-6 space-y-5">
              ...
              <div className="flex gap-3">
                <button
                  onClick={handleSave}
                  disabled={saving}
                  className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-primary py-2.5 text-sm font-bold text-white hover:opacity-90 disabled:opacity-50"
                >
                  {saving ? (
                ...
                <button
                  ...
                  className="flex-1 rounded-xl border border-default py-2.5 text-sm font-semibold text-secondary hover:bg-subtle"
                >
                  Cancel
                </button>
              </div>
            ...
            <div className="rounded-2xl border border-default bg-white p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                {[
                  ...
                  },
                  {
                    ...
                    value: (
                      assignment.allowed_submission_types || ["text"]
                    ).join(", "),
                  },
                ].map(({ label, value }) => (
                  <div key={label}>
              ...
              {assignment.instructions && (
                <div>
                  ...
                  <div
                    className="rounded-xl border border-default bg-subtle p-4 text-sm text-secondary rich-content leading-relaxed"
          ...
          <div className="rounded-2xl border border-default bg-white p-5 shadow-sm">
            ...
            <div className="space-y-2">
              ...
              <button
                ...
                className="flex w-full items-center justify-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-bold text-white hover:opacity-90 disabled:opacity-50"
              >
                {assigning ? (
                  <RoboLoader size="xs" className="text-current" />
                ) : (
              ...
              <button
                ...
                className="flex w-full items-center justify-center gap-2 rounded-xl border border-default px-4 py-2 text-sm font-semibold text-secondary hover:bg-subtle disabled:opacity-50"
              >
