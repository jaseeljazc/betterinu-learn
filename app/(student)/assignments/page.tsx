...
export default function MyAssignmentsPage() {
  ...
  const isLoading =
    standaloneAssignments === null || courseAssignments === null;
  const newOtherCount =
    ...
    <div className="flex items-center gap-4 rounded-2xl border border-default bg-white p-4 shadow-sm">
      ...
      <div className="min-w-0 flex-1">
        ...
        {status === "rejected" && a.feedback && (
          <p className="mt-1 text-xs text-red-600 bg-red-50 rounded-lg px-2 py-1 truncate">
            Feedback: {a.feedback}
          </p>
        )}
      </div>
      <span
        className={`shrink-0 inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide ${cfg.cls}`}
      >
        <cfg.Icon size={11} />
        {cfg.label}
      </span>
